...
async def admin_create_game(
    body: GameCreate, session: AdminTenantSessionDep, tenant_id: AdminTenantIdDep
) -> GameOut:
    return GameOut.model_validate(await create_game(session, tenant_id, body))
...
async def admin_update_game(
    ...
    return GameOut.model_validate(await update_game(session, tenant_id, game_id, body))
...
async def admin_delete_game(
    game_id: uuid.UUID, session: AdminTenantSessionDep, tenant_id: AdminTenantIdDep
) -> None:
