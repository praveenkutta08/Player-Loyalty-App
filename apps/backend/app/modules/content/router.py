...
async def admin_create_content(
    body: ContentCreate, session: AdminTenantSessionDep, tenant_id: AdminTenantIdDep
) -> ContentItemOut:
    return ContentItemOut.model_validate(await create_item(session, tenant_id, body))
...
async def admin_update_content(
    ...
    return ContentItemOut.model_validate(await update_item(session, tenant_id, item_id, body))
...
@router.post(
    ...
    response_model=ContentItemOut,
    tags=["content"],
    dependencies=[Depends(require(Permission.content_publish.value))],
)
async def admin_publish_content(
    item_id: uuid.UUID, session: AdminTenantSessionDep, tenant_id: AdminTenantIdDep
) -> ContentItemOut:
    return ContentItemOut.model_validate(await publish_item(session, tenant_id, item_id))
