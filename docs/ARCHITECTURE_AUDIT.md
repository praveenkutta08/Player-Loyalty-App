...
> **RESOLVED (Android R9; iOS R20, 2026-07-03).** Android per-tenant product flavors + `TenantBuildConfigModule` + bundled fonts ship both `demoDebug` and `auroraBayDebug` APKs from one JS bundle (R9). iOS (R20): the repo side is fully wired — `Info.plist` sources `CFBundleDisplayName`/`TenantId`/`TenantSlug`/`TenantApiBaseUrl` from the attached xcconfig and declares `UIAppFonts` for the six bundled brand TTFs; `Demo`/`AuroraBay` xcconfigs + the `TenantBuildConfig` Swift/ObjC module are committed. The remaining `project.pbxproj` steps (per-tenant configurations with `baseConfigurationReference`, shared schemes, target membership + bridging header, Copy Bundle Resources) are Mac-only per CLAUDE.md and documented as an exact click-by-click runbook in `docs/WHITE_LABEL_CUSTOMIZATION_ROADMAP.md` §6/§6.1. `buildConfig.ts` reads all of this via the native module with `.env`/demo fallbacks. Test: `iosPackaging.test.ts` (Info.plist ↔ xcconfig ↔ font wiring guard).
...
- **M2. Cursor pagination built but never used** — **RESOLVED (R14 audit feed + players; R19 the rest, 2026-07-03).** `core/pagination.py` now exposes a generic `paginate_keyset` (keyset on a datetime + id, opaque cursor, `limit+1` look-ahead). Rolled out to every high-volume list: wallet transactions, redemptions, admin offers/promotions, rewards catalog, CMS content, and geofence zones — each returns a concrete `*Page` envelope in the OpenAPI contract. Admin/mobile consumers unwrap `.items` via `transformResponse` and request a full first page (limit 100); infinite-scroll "load more" following `next_cursor` is a documented UI follow-up needing no further backend change. The small curated `/app/*` player feeds (active offers/content/beacons) stay full lists by design. Test: `test_wallet.py::test_transaction_history_cursor_pagination` (keyset paging to exhaustion, no overlap/gaps).
...
- **M10. RLS test coverage is one synthetic-table test** — **RESOLVED (R14 offers/rewards/players; R21 wallet, 2026-07-03).** `test_cross_tenant_api.py` probes each resource through the real request stack; R21 added the direct wallet probe (a tenant-A player session sees an empty ledger + zero balance while tenant B is funded), closing the last indirect gap.
...
> **RESOLVED (R16, 2026-07-03):** egm/pair TODO documented; migration chain verified (linear, current==head, stale `.pyc` cleaned; `__pycache__` gitignored); wallet reconciliation via `POST /wallet/admin/reconcile` (audited, cron-callable); P6 vars added to `.env.example`; `/concierge/ask` rate-limited per player + narration output post-check drops ungrounded $/% figures; admin Modal has focus trap + `aria-labelledby` + focus restore, dead ⌘K removed; promotions calendar opens on the current month; Offers/Wallet distinguish error vs empty; `casino://` Android intent-filter + `useDeepLinks`; mobile `analyticsApi` types from the generated schema; `GETTING_STARTED.md` marked historical; dead `useIsScopedAdmin` removed. **Deferred → now RESOLVED (R21, 2026-07-03):** CI coverage thresholds are enforced — backend `pytest --cov` gates on `fail_under=80` (current ~85%), mobile Jest on a 60/50/50/60 floor (current ~67%), admin Vitest on a scoped logic floor (current ~11% stmts / 66% branches); all wired into the `web`/`backend` CI jobs. The tracked `tsconfig.tsbuildinfo` is untracked and `*.tsbuildinfo` gitignored. **Still deferred (LOW):** mobile manifest `If-None-Match` (needs shared-baseQuery 304 handling).
