...
The demo tenant's UUID is generated by the DB, so a fresh seed produces a **new** id. Copy the
`tenant_id` printed in step 3 into **`apps/mobile/src/config/buildConfig.ts`**:

```ts
export const buildConfig: BuildConfig = {
  // …
  tenantId: '<paste the printed tenant_id here>',
  // …
};
```

Skip this and the app fetches a manifest for an unknown tenant (no branding / 404). Re-seeding an
existing DB is idempotent and keeps the same id.
...
- **Mobile shows no branding / manifest 404** → `buildConfig.tenantId` doesn't match the seeded
  tenant. Re-do step 4.
